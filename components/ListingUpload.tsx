  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const propertyToSave = {
        ...
        isAvailable: true,
        amenities: initialData?.amenities || ['Verified Listing', 'Professional Management'],
      ...
      if (initialData?.id) {
        // Update existing
        await setDoc(doc(db, "properties", initialData.id), propertyToSave, { merge: true });
      } else {
        // Add new
        await addDoc(collection(db, "properties"), propertyToSave);
      }
      
      setProgress(100);
      setTimeout(() => {
        onSuccess(propertyToSave);
        onClose();
      }, 500);

    } catch (error) {
      console.error(error);
      alert("Database error. Please check your Firebase configuration.");
      setUploading(false);
    }
  };
        ...
        <div className="p-8 sm:p-12">
          <div className="flex justify-between items-center mb-10">
            <div>
              ...
              <p className="text-xs font-black uppercase tracking-[0.3em] text-brand-gold mt-2">Cloud Persistence Portal</p>
            </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-widest text-gray-500 ml-1">Price (KES)</label>
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-widest text-gray-500 ml-1">Bedrooms</label>
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase tracking-widest text-gray-500 ml-1">Media (Photo/Video)</label>
              <div className="relative group overflow-hidden rounded-2xl">
                ...
                <div className={`border-2 border-dashed rounded-2xl p-8 text-center transition-all ${file ? 'border-brand-gold bg-brand-gold/5' : 'border-white/10 bg-white/5 hover:border-brand-gold/30'}`}>
                   <p className="text-xs font-bold text-gray-500">{file ? file.name : "Tap to browse cloud media"}</p>
                </div>
              </div>
            </div>
            ...
            <div className="pt-6">
              <button type="submit" disabled={uploading} className="w-full py-5 gold-bg-gradient text-brand-black font-black uppercase tracking-widest rounded-2xl shadow-2xl shadow-brand-gold/10 hover:brightness-110 transition-all active:scale-[0.98]">
                {uploading ? "Syncing to Cloud..." : "Authorize Publication"}
              </button>
            </div>
          </form>
        </div>
